...
export class SafeApplyService {
 ...
 async apply(policies:NetworkPolicy[],timeoutMs=60_000,initiatedBy?:string){if(timeoutMs<5_000||timeoutMs>300_000)throw new ValidationError("rollback window must be between 5 and 300 seconds");const commitId=this.ids.next();await this.enforce.stage(commitId,policies,initiatedBy);await this.bus.publish("security.policy.staged",{commitId,at:this.clock.now(),timeoutMs});
